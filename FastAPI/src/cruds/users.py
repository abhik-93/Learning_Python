...
from ..utils.hashed_pwd import encode_to_hash_pwd
...
def create_user(db: Session, new_user: PyUsers):
    new_user_obj = Users(name=new_user.name, email=new_user.email, password=encode_to_hash_pwd(new_user.password))
    db.add(new_user_obj)
    db.commit()
